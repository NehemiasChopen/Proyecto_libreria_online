...
if (!empty($_POST["actualizar"])){//Comprobacion de boton registro
    if (!empty($_POST["id_u"]) and !empty($_POST["nombre"]) and !empty($_POST["correo"]) and !empty($_POST["direccion"]) and !empty($_POST["rol"]) and !empty($_POST["password"])){//Comprobacion de campos vacios
        ...
        if($r=1){
           echo '<div class="alert alert-success d-flex align-items-center">Se actuliazaron los datos con exito</div>';//Mensaje de confirmacion
        }else{
